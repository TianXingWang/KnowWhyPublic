...
import com.txwang.knowwhy.mapper.QuestionMapper;
...
//import com.txwang.knowwhy.model.entity.QuestionFavour;
//import com.txwang.knowwhy.model.entity.QuestionThumb;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    @Override
    public Page<Question> searchFromDB(QuestionQueryRequest questionQueryRequest) {
        long current = questionQueryRequest.getCurrent();
        long size = questionQueryRequest.getPageSize();
        // 题目表的查询条件
        QueryWrapper<Question> queryWrapper = this.getQueryWrapper(questionQueryRequest);
        queryWrapper.like("title", questionQueryRequest.getSearchText());
        queryWrapper.like("content", questionQueryRequest.getSearchText());
        queryWrapper.like("answer", questionQueryRequest.getSearchText());
    ...
    @Override
    public String getAiQuestionAnswers(Question question, HttpServletRequest request) {
        ThrowUtils.throwIf(question == null, ErrorCode.PARAMS_ERROR);
        String userPrompt = question.getContent();

        //字符串操作
        RBucket<String> rBucket =  redissonClient.getBucket("question:aianswer:" + question.getId());

        if (rBucket.get() != null) {
            return rBucket.get();
        }

        String aiAnswer = aiManager.startChat(userPrompt);
        // 设置value和key的有效期
        rBucket.set(aiAnswer, 1, TimeUnit.DAYS);
        return aiAnswer;
    }
